			...
			for ( int i = 0 ; ( i < Items . Count ) && ( currentHeight < finalRect . Height ) ; i++ )
			{
				...
				switch ( this [ control ] )
				{
					...
					case ContentAlign . Center:
						{
							...
							control . Arrange ( new Rectangle ( finalRect . LeftTopPoint . Offset ( ( control . DesiredSize . Width - controlWidth ) / 2 , currentHeight ) ,
					...
					case ContentAlign . Right:
						{
							int controlWidth = Math . Min ( finalRect . Width , control . DesiredSize . Width );
							control . Arrange ( new Rectangle ( finalRect . LeftTopPoint . Offset ( control . DesiredSize . Width - controlWidth , currentHeight ) ,
																new Size ( controlWidth ,
																			Math . Min ( Math . Max ( finalRect . Height - currentHeight , 0 ) , control . DesiredSize . Height ) ) ) );
							break;
						}
